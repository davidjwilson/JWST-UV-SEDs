...
path = '/home/david/work/meats/SEDs/draft_hlsp/toi-178/'  #path where the files are

star = 'TOI-178'
version = 1
airglow =  [1214, 1217, 1300, 1310, 1353, 1356]

trims = {'G430L':[3140, 5690], 'G230L':[1710, 3140], 'G140L':[1150, 1710], 'apec':[0, 50]}

def make_sed(path, star, version, norm=False, remove_negs=False, to_1A=False, sed_type='var'):
    
    sed_table = []
    instrument_list = []
    sed_table, instrument_list = sed.add_stis_and_lya(sed_table, path, airglow[0:2], instrument_list, airglow[2:], norm=True, remove_negs=remove_negs,to_1A=to_1A, trims=trims, lya_max = True, optical=True)
