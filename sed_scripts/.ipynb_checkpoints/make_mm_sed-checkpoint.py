...
def fill_model(table, model_name): 
    ...
    if 'ERROR' not in table.dtype.names:
        table['ERROR'] = fill_zeros
...
def add_stis_and_lya(sed_table, component_repo, lya_range, instrument_list, other_airglow, norm=False, error_cut=True, optical = False, remove_negs=False, to_1A=False, do_airglow=True, trims = {}):
    """
   ...
   # g140m_path = glob.glob(component_repo+'*g140m*.ecsv')
    ...
    for grating in stis_gratings:
        ...
        if len(specpath) == 1:
            ...
            if len(sed_table) == 0:
                sed_table = dict(WAVELENGTH = [0]) #bit of a hack, improve later
            if data.meta['INSTRUME'] =='STIS':
                ...
                if grating in trims:
                    ...
                    data = data[mask]
                if grating == 'E140M' or len(sed_table['WAVELENGTH'])==1:
                    sed_table = data
                else:                
    ...
    if  uses_e140m == False and used_g140l == False and do_airglow:
...
def add_phoenix_and_g430l(sed_table, component_repo, instrument_list, error_cut=True, scale=False, remove_negs=False, to_1A=False, trims={}):
    ...
    if len(phx_path) == 1 and len(g430l_path) == 1:
       ...
       # print(phx.meta['NORMFAC'])
        ...
        if 'ERROR' in phx.dtype.names:
            phx['ERROR'] *=phx.meta['NORMFAC']
