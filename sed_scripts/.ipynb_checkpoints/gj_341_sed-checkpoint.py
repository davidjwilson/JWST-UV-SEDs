...
path = '/home/david/work/meats/SEDs/draft_hlsp/gj_341/'  #path where the files are
...
def make_sed(path, star, version, norm=False, remove_negs=False, to_1A=False, sed_type='var'):
    ...
    proxy_path = '/media/david/2tb_ext_hd/hddata/mega_muscles/v24_hlsp/GJ676A/hlsp_muscles_hst_stis_gj676a_g140l_v24_component-spec.fits'
    d_prox = 16.0272
    d_star = 10.45
    scale = (d_prox/d_star)**2
    sed_table, instrument_list = sed.add_proxy(sed_table, proxy_path, instrument_list, scale, ranges = [1367, 1624], remove_negs=remove_negs,to_1A=to_1A)
