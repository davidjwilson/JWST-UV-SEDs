...
path = '/home/david/work/meats/SEDs/draft_hlsp/wasp-166/'  #path where the files are

star = 'WASP-166'
version = 1
airglow =  [1214, 1217, 1300, 1310, 1353, 1356]

trims = {'G430L':[3141, 5690], 'G230L':[1712, 3140], 'G140L':[1170, 1711]}

def make_sed(path, star, version, norm=False, remove_negs=False, to_1A=False, sed_type='var'):
    
    sed_table = []
    instrument_list = []
    sed_table, instrument_list = sed.add_stis_and_lya(sed_table, path, airglow[0:2], instrument_list, airglow[2:], norm=False, remove_negs=remove_negs,to_1A=to_1A, trims=trims, lya_max = True, optical=True, Ebv=0.03)
    ...
    sed_table, instrument_list, gap = sed.add_xray_spectrum(sed_table, path, instrument_list, 'cxo', add_apec = True, find_gap=True, to_1A=to_1A, trims=trims)
