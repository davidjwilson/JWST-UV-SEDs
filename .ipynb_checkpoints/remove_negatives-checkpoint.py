...
def make_clean_spectrum(spectrum, dv=0*u.km/u.s):
 ...
 'EXPEND' 
    ...
    #testing interpolating back onto old grid
    new_flux = interpolate.interp1d(new_wavelength, new_flux, fill_value='extrapolate')(spectrum['WAVELENGTH'])
    new_error = interpolate.interp1d(new_wavelength, new_error, fill_value='extrapolate')(spectrum['WAVELENGTH'])
    
    print(len(spectrum['WAVELENGTH']), len(new_flux))
    
    new_wavelength = spectrum['WAVELENGTH']
...
#                            new_dq,new_expstart*cds.MJD, new_expend*cds.MJD], names=names, meta= spectrum.meta)
    ...
    new_spectrum = Table([new_wavelength*u.AA, new_flux*u.erg/u.s/u.cm**2/u.AA, new_error*u.erg/u.s/u.cm**2/u.AA], names=['WAVELENGTH', 'FLUX', 'ERROR'])
    return new_spectrum
