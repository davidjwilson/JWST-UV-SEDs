...
Turns eROSISTA data in to HLSP format ecsv and fits files, ready to be added to an SED. MEATS version. Based mostly on prepare xmm. V1 will be a bit of a bare minimum fudge because we're only using it for one star.
"""
...
def build_erosita_metadata(hdr0, new_data, hlsp):
    """
    ...
    meta_names =['TELESCOP','INSTRUME','GRATING','DETECTOR','FILTER',
                 ...
                 'EXPMAX','EXPMED','NORMFAC','WAVEMIN','WAVEMAX','WAVEUNIT','AIRORVAC','SPECRES','WAVERES','FLUXMIN',
                  'FLUXMAX','FLUXUNIT']
    meta_fill = ['eROSITA','NA','NA','NA', 'NA',
                 hdr0['SKYFIELD'], hdr0['NINST'], hdr0['DSCODE'],
    hdr0['TARGET'].replace(' ','_'),'','',hlsp['PROPOSID'][0],
                 hlsp['PROGRAM'][0],hlsp['PROGRAMSHORT'][0],hlsp['HLSPLEAD'][0],
                 hlsp['PR_INV_L'][0],hlsp['PR_INV_F'][0] ,hdr0['DATE-OBS'], start, end, exptime, 'MEAN', exptime, 
...
def make_component_filename(metadata, version):
    ...
    """
    filename = 'hlsp_muscles_%s_%s_%s_%s_v%s_component-spec' %('ero', metadata['INSTRUME'].lower(), metadata['TARGNAME'].lower(), metadata['GRATING'].lower(), version) 
    return filename
...
def make_dataset_extension(hdr):
    ...
    """
    description_text = 'This extension contains a list of DSCODES (DATASET_ID used for consistency with HST data) that can be used to locate the data in the erosita archives. '

    rootnames = [hdr['DSCODE']]
    datasets = ['']
...
def make_erosita_spectra(erosita_path, savepath, version, hlsp, apec_repo='', make_apec=True, save_ecsv=False, save_fits=False):
    hdul = fits.open(erosita_path)
    hdr0 = hdul[0].header
    data = hdul[1].data
    if make_apec:
    ...
    data = build_erosita_data(data, hdr0)
    metadata = build_erosita_metadata(hdr0, data, hlsp)
    print(metadata)
    if make_apec:
