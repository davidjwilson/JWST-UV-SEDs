...
for x in xtab:
    if x['Star'] == 'TOI-260':
        ...
        while n < ntries:
            ...
            if x['tel'] == 'xmm':
                AllModels.setEnergies("0.102 2.5 6901")
            else:
        ...
        if len(starx) > 0:
            ...
            if len(starx) > 0: #TOI-260 has a much longer model for some reason, cutting it to the same as the others for consistencey
                print('found the txt')
                hlsp= True
        ...
        if hlsp:
            ...
            savdat = Table((wavelength*u.AA, fluxmean*u.erg/u.s/u.cm**2/u.AA , fluxstd*u.erg/u.s/u.cm**2/u.AA), 
                           names=['WAVELENGTH', 'FLUX', 'ERROR'])
        ascii.write(savdat, '../models/{}_apec_errs_test.ecsv'.format(x['Star']), format='ecsv', overwrite=True)
